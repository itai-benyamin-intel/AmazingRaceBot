...
class TestPhotoVerificationTimeoutWorkflow(unittest.TestCase):
    ...
    def test_workflow_with_photo_verification_and_penalty(self):
        ...
        game_state.use_hint("Team Alpha", 1, 0, 1, "Alice")
        
        # Team completes Challenge 1 (answer-based challenge)
        print("\n--- Team completes Challenge 1 ---")
        completion_result = game_state.complete_challenge("Team Alpha", 1, 3)
        ...
        unlock_time = game_state.get_challenge_unlock_time("Team Alpha", 2)
        ...
        verification_id = game_state.add_pending_photo_verification(
            "Team Alpha", 2, "photo_12345", 1, "Alice"
        )
        ...
        unlock_time_str = game_state.get_challenge_unlock_time("Team Alpha", 2)
        ...
        self.assertLess(penalty_diff, 5,
                       f"Unlock time should be ~2 minutes from approval. Got {penalty_diff}s difference")
        print(f"✓ Step 6 PASS: Penalty timeout is 2 minutes from photo approval")
    ...
    def test_workflow_without_photo_verification(self):
        ...
        game_state.use_hint("Team Beta", 1, 0, 2, "Bob")
        
        # Team completes Challenge 1
        print("\n--- Team completes Challenge 1 (photo verification disabled) ---")
        completion_time_before = datetime.now()
        completion_result = game_state.complete_challenge("Team Beta", 1, 3)
        self.assertTrue(completion_result)
        ...
        unlock_time_str = game_state.get_challenge_unlock_time("Team Beta", 2)
        self.assertIsNotNone(unlock_time_str,
                            "Step 3 FAIL: Unlock time should be calculated after completion")
        
        unlock_time = datetime.fromisoformat(unlock_time_str)
        expected_unlock = completion_time_before + timedelta(minutes=2)
        penalty_diff = abs((unlock_time - expected_unlock).total_seconds())
        self.assertLess(penalty_diff, 5,
                       f"Unlock time should be ~2 minutes from completion. Got {penalty_diff}s difference")
        print(f"✓ Step 3 PASS: Penalty timeout is 2 minutes from challenge completion")
